...
| **Asset Type** | Hibah Award |
| **Control Record** | Hibah Award Procedure |
...
| POST | `/v1/hibah-award-procedure/initiate` | Initiate |
| GET | `/v1/hibah-award-procedure` | Retrieve (list) |
| GET | `/v1/hibah-award-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/hibah-award-procedure/{crId}/update` | Update |
| PUT | `/v1/hibah-award-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/hibah-award-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-hibah-discretionary-gift:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-hibah-discretionary-gift ./helm -n isb-deposits
```

Exposed through the platform Gateway at path prefix `/isb-hibah-discretionary-gift` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
